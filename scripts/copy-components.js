...
const SOURCE_DIR = path.join(process.cwd(), 'example', 'components');
const TARGET_DIR = path.join(process.cwd(), 'website', 'src', 'preview-components');

async function copyComponents() {
  console.log('📋 Copying example components for local development...\n');

  // Check if source directory exists (it won't in npm package installations)
  if (!fs.existsSync(SOURCE_DIR)) {
    console.log('⚠️  No example components found - skipping copy');
    console.log('   (This is expected when building from npm package)\n');

    // Create empty index.ts for clean builds
...
`;
    fs.writeFileSync(path.join(TARGET_DIR, 'index.ts'), emptyIndex, 'utf-8');
    console.log('✅ Created empty component map (no example components to copy)\n');
    return;
  }
  ...
  console.log(`\n✅ Copied ${copiedCount} files to website/src/preview-components`);
}
